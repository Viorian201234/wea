...
async function starts() {
	...
	fs.existsSync('./Cuans.json') && client.loadAuthInfo('./Cuans.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Cuans.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6281292091364@s.whatsapp.net"] //Mau ganti no hp lu disini nanti
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
				...
				data = await fetchJson(`https://cuans-apis.herokuapp.com/api/randomquotes`)
				ez = `*➸ Author :* ${data.author}\n*➸ Quotes :* ${data.quotes}`
				reply(ez)
                                await limitAdd(sender)
				break
				case 'lolmaker':
                data = await await getBuffer(`https://api-cuans.herokuapp.com/lolmaker?text=${body.slice(8)}`)
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Sikintil", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Nih nomor ownerku ngab, kalo mau gelud chat aja kalo ngga serlok',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				case 'artinama':
                  ...
                  if (!isRegister) return reply(mess.only.daftarB)
                  if (isLimit(sender)) return reply(ind.limitend(pusname))
                    data = await fetchJson(`https://cuans-apis.herokuapp.com/api/arti?nama=${body.slice(10)}`)
                   reply(data.result)
                   await limitAdd(sender)
                   break
				case 'wiki':
					...
					tels = body.slice(6)	
                                        if (!isRegister) return reply(mess.only.daftarB)				
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/wiki?q=${tels}`, {method: 'get'})
					reply(anu.result)
                                        await limitAdd(sender)
					break				
				/* case 'ytmp3':
				...
				case 'ytmp4':
					...
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.titleInfo}.mp4`, quoted: teks})
                                        await limitAdd(sender)
					break
				case 'testime':
					setTimeout( () => {
				...
				case 'send':
					...
					case 'quotesnime':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					nimek = await fetchJson('https://cuans-apis.herokuapp.com/api/quotesnime/random')
					hasil = `*Anime* : ${nimek.data.anime}\n*Character* : ${nimek.data.character}\n*Quotes* : ${nimek.data.quote}\n*Creator* : ${nimek.creator}`
					reply(hasil)
                                        await limitAdd(sender)
					break
				case 'setppbot':
				...
				case 'chord':
					if (args.length < 1) return reply('judul lagunya mana kak')
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
					anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/chord?q=${tels}`, {method: 'get'})
					reply(anu.result)
                                        await limitAdd(sender)
					break
				case 'lirik':
			...
			/*case 'igstalk':
                      if (!isRegister) return reply(mess.only.daftarB)
                      if (isLimit(sender)) return reply(ind.limitend(pusname))
                      hmm = await fetchJson(`https://cuans-apis.herokuapp.com/api/stalkigeg?username=${body.slice(9)}`)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				case 'neko':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/nekonime`, {method: 'get'})
					buffer = await getBuffer(anu.result)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
             ...
             /*            case 'play':   
	          if (!isRegister) return reply(mess.only.daftarB)
                ...
                reply(mess.wait)
                play = body.slice(5)
                anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/play?query=Armada%20tentangmu${play}`)
               if (anu.error) return reply(anu.error)
				...
				case 'pantun':
					...
					anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/randompantun`, {method: 'get'})
					reply(anu.pantun)
                                        await limitAdd(sender)
					break		
			    case 'waifu':
					...
					reply(mess.wait)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/waifu`, {method: 'get'})
            ...
            break
				case 'fitnah':
				if (args.length < 1) return reply(`cara pakai :\n${prefix}fitnah [@tag|pesan|balasanbot]]\n\nEx : \n${prefix}fitnah @tagmember|hai|hai juga`)
				var gh = body.slice(7)
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
            ...
            break
                                case 'infogempa':
                                        anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/infogempa`, {method: 'get'})
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        if (anu.error) return reply(anu.error)
				...
				case 'clone':
					...
					break
                                case 'faktaunik':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://cuans-apis.herokuapp.com/api/fakta`, {method: 'get'})
                                        reply(anu.result)
                                        await limitAdd(sender)
                                        break
                                case 'bal':
                                ...
                                case 'daftar':
                                        ...
                                        if (isGroup) {
                                                ...
                                                console.log(color('---[DAPTAL]---'), color(time, 'yellow'), 'Name:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                                        } else {
                                                addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                                                await reply(ind.registered(namaUser, umurUser, serialUser, time, sender))
                                                addATM(sender)
                                                addLevelingId(sender)
                                                console.log(color('---[DAPTAL]---'), color(time, 'yellow'), 'Name:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
                                        }
                                        break
                                case 'mining':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[WARN]','red'), 'Perintah Tidak Terdaftar dari', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
